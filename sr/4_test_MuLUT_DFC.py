            ...
            for mode in self.opt.modes:
                ...
                for r in [0, 1, 2, 3]:
                    ...
                    pred += FourSimplexInterpFaster_compress(self.lutDict[key1],self.lutDict[key2], img_in, h, w,
                                                    self.opt.interval, 4 - r, self.opt.cd, self.opt.dw, self.opt.si-self.opt.interval,upscale=upscale, out_c = 1, mode=mode,
    ...
    for s in range(opt.stages):
        ...
        for mode in opt.modes:
            ...
            lutPath = os.path.join(opt.expDir,
                                   "{}_{}.npy".format(opt.lutName, key))
            lutDict[key] = np.load(lutPath).astype(np.float32).reshape(-1, v_num)

            if compression_type =='xy':
                lutDict[key] = np.load(lutPath).astype(np.float32).reshape((-1,L,L, v_num))
            elif compression_type == 'xyz':
                lutDict[key] = np.load(lutPath).astype(np.float32).reshape((-1, L, v_num))
            else:
