...
def InterpTorchBatch_compress_xy(weight, img_in, h, w, interval, rot, upscale=4, out_c=1, mode='s',d=2,ref2index=None):
    q = 2 ** interval  # 16
    L = 2 ** (8 - interval) + 1  # 17
...
def InterpTorchBatch_compress_xyz(weight, img_in, h, w, interval, rot, upscale=4, out_c=1, mode='s',d=2,ref2index=None):
    q = 2 ** interval  # 16
    L = 2 ** (8 - interval) + 1  # 17
...
def FourSimplexInterpFaster_compress(weight_c1,weight_c2, img_in, h, w, interval, rot, upscale=4, out_c=1, mode='s',d = dwidth,ref2index=None):
    if compression_type == 'xy':
        out_compress1, index_flag = InterpTorchBatch_compress_xy(weight_c1, img_in, h, w, interval, rot, upscale,
                                                                   out_c,mode, d=d, ref2index=ref2index)
    elif compression_type == 'xyz':
        out_compress1, index_flag = InterpTorchBatch_compress_xyz(weight_c1, img_in, h, w, interval, rot, upscale,
                                                                 out_c, mode, d=d, ref2index=ref2index)
    else:
            ...
            for mode in self.opt.modes:
                ...
                for r in [0, 1, 2, 3]:
                    ...
                    pred += FourSimplexInterpFaster_compress(self.lutDict[key1], self.lutDict[key2], img_in, h, w,
                                                    self.opt.interval, 4 - r, upscale=1,out_c=out_c_list[s], mode=mode,
...
if __name__ == "__main__":
    ...
    for mode in opt.modes:
        ...
        lut_path = os.path.join(opt.expDir, 'weight_s{}c0_{}_compress1.npy'.format(1, mode))
        ...
        lut_path = os.path.join(opt.expDir, 'weight_s{}c0_{}_compress2.npy'.format(1, mode))
        ...
        lut_path = os.path.join(opt.expDir, 'weight_s{}c0_{}_compress1.npy'.format(2, mode))
        ...
        lut_path = os.path.join(opt.expDir, 'weight_s{}c0_{}_compress2.npy'.format(2, mode))
        ...
        lut_path = os.path.join(opt.expDir, 'weight_s{}c0_{}_compress1.npy'.format(3, mode))
        ...
        lut_path = os.path.join(opt.expDir, 'weight_s{}c0_{}_compress2.npy'.format(3, mode))
        ...
        lut_path = os.path.join(opt.expDir, 'weight_s{}c0_{}_compress1.npy'.format(4, mode))
        ...
        lut_path = os.path.join(opt.expDir, 'weight_s{}c0_{}_compress2.npy'.format(4, mode))
        ...
        for c in range(4):
            ...
            lut_path = os.path.join(opt.expDir, 'weight_s{}c{}_{}_compress1.npy'.format(6,c, mode))
            ...
            lut_path = os.path.join(opt.expDir, 'weight_s{}c{}_{}_compress2.npy'.format(6, c, mode))
