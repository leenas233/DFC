...
if __name__ == "__main__":
    ...
    c_in = 1
    model_G = model(lut_folder=opt.expDir, modes=modes, stages=stages, interval=opt.interval,
    ...
    if opt.model in ['SPF_LUT_DFC', 'SPF_LUT']:
        ...
        for s in range(stages):
            ...
            for c in range(c_in):
                for mode in modes:
                    ft_lut_path = os.path.join(opt.expDir,"LUT_ft_{}bit_int8_s{}_c{}_{}_compress1.npy".format(opt.interval,str(stage),str(c), mode))
                    ...
                    ft_lut_path = os.path.join(opt.expDir,"LUT_ft_{}bit_int8_s{}_c{}_{}_compress2.npy".format(opt.interval,str(stage),str(c), mode))
